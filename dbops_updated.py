...
class Movie():
    ...
    def add_movie_from_omdb_data(file_path, omdb_data):
        ...
        with closing(conn.cursor()) as cursor:
            ...
            cursor.execute("""
                INSERT OR IGNORE INTO movie (file_path, title, date_created, imdb_id)
                VALUES (?, ?, ?, ?)
...
class Frame():
    ...
    def compute_frame_aggregated(self):
        # Get the frame image once
        frame_image = self.get_frame_image()
        if not frame_image:
            raise Exception("No image to compute hash on")
        
        # Compute all necessary hashes and other attributes
        wavelet_hash = str(imagehash.whash(frame_image))
        a_hash = str(imagehash.average_hash(frame_image))
        d_hash = str(imagehash.dhash(frame_image))
        perceptual_hash = str(imagehash.phash(frame_image))
        md5_hash = hashlib.md5(frame_image.tobytes()).hexdigest()
        average_color = array_to_blob(np.mean(np.array(frame_image), axis=(0, 1)))  # Convert to blob
        
        # Open cursor and batch the updates
        cursor = conn.cursor()
        try:
            # Update all the hash values and other frame attributes in a single query
            cursor.execute("""
                UPDATE frame
                SET 
                    wavelet_hash = ?, 
...
class Face():
    ...
    def add_face(imdb_id: str, frame_index: int, face_index: int, 
                 facial_landmarks=None, emotion_embedding=None, face_embedding=None):
        ...
        with closing(conn.cursor()) as cursor:
            ...
            cursor.execute("""
                INSERT OR IGNORE INTO face (imdb_id, frame_index, face_index, facial_landmarks, 
                    emotion_embedding, face_embedding)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (imdb_id, frame_index, face_index, 
                  array_to_blob(facial_landmarks) if facial_landmarks is not None else None,
                  array_to_blob(emotion_embedding) if emotion_embedding is not None else None,
                  array_to_blob(face_embedding) if face_embedding is not None else None))
    ...
    def compute_face_aggregated(self):
        # Get the aligned face image once
        frame_image = self.get_face_image_aligned()
        if not frame_image:
            raise Exception("No image to compute hash on")
        
        # Compute hashes
        wavelet_hash = imagehash.whash(frame_image)
        a_hash = imagehash.average_hash(frame_image)
        d_hash = imagehash.dhash(frame_image)
        perceptual_hash = imagehash.phash(frame_image)
        md5_hash = hashlib.md5(frame_image.tobytes()).hexdigest()
        
        # Update the database
        with closing(conn.cursor()) as cursor:
            cursor.execute("""
                UPDATE face SET wavelet_hash = ?, a_hash = ?, d_hash = ?, perceptual_hash = ?, md5_hash = ?
                WHERE imdb_id = ? AND frame_index = ? AND face_index = ?
            """, (str(wavelet_hash), str(a_hash), str(d_hash), str(perceptual_hash), md5_hash, self.imdb_id, self.frame_index, self.face_index))
        conn.commit()
